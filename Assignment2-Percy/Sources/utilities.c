...
//this function will move spindle to the correct place to draw shapes
void fly_spindle(int mode){
	if (mode == 2){
		for(int i = 0; i<=40; i++){
			Dir_z_PutVal(1);
			driveMotorZ(600);
...
void GUI_shapes() {
	...
	Term1_MoveTo(5, 18);
	Term1_SendStr("O >>>>    Olmpic");
	Term1_MoveTo(5, 20);
	Term1_SendStr("Q>>>>     Quit");
	Term1_MoveTo(5, 22);
	Term1_SendStr("Attention: leave 20 steps for spindle far away from sheet!");
}
